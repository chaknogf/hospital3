...
export class FormularioPacienteComponent implements OnInit, OnDestroy {
  ...
  private manejarDatosRenap(): void {
    ...
    if (pacienteRenap) {
      ...
      if (pacienteRenap.fecha_nacimiento) {
        ...
        const rn = this.pacienteUtil.validarRecienNacido(
          pacienteRenap.fecha_nacimiento
        );
        this.form.get('edad')?.patchValue(edad);
        this.esRecienNacido.set(rn.recienNacido);
      }
    }
  ...
  private cargarPacienteParaEdicion(): void {
    ...
    this.api.getPaciente(id)
      ...
      .subscribe(data => {
        ...
        try {
          // console.log('✅ Paciente obtenido del backend:', data);
          ...
          // console.log('📝 Paciente convertido para formulario:', pacienteParaForm);
          ...
          if (data.fecha_nacimiento) {
            ...
            const rn = this.pacienteUtil.validarRecienNacido(data.fecha_nacimiento);
            this.esRecienNacido.set(rn.recienNacido);
          }
  ...
  private configurarSuscripciones(): void {
    ...
    this.form.get('contacto.departamento')?.valueChanges
      .pipe(takeUntil(this.destroy$))
      .subscribe(() => this.listarMunicipiosDireccion());
  }
  ...
  listarMunicipiosDireccion(): void {
    try {
      ...
      this.municipios_direccion.set(municipios_filtrados);
    } catch (error) {
      this.mostrarError('obtener municipios', error);
      this.municipios_direccion.set([]);
    }
